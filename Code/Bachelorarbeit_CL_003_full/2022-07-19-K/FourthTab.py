...
class FourthTab(QWidget):
    ...
    def __init__(self):
        ...
        self.max_word_label = QLabel(self)              # Label maximum words in summary
        self.max_word_label.setText("Maximum words:")   # Label maximum words in summary
        self.max_words = QLineEdit()                    # QlineEdit (user input) maximum words in summary
        self.max_words.setMaxLength(5)
        self.max_words.setFixedWidth(100)               # ToDO: does not scale well with changed width
        ...
        self.grid_import.addWidget(self.max_word_label,2,0)
        self.grid_import.addWidget(self.max_words,2,1)
        ...
        # Button Signal/Slots for class FourthTab
        self.btn_import_file.clicked.connect(lambda: self.open_file_and_path(0))
    ...
    def open_file_and_path(self, int):
        case = int
        input_text, input_text_path = helper.file_open_return_helper(self)
        #input_text_path = helper.file_open_return_helper(self)[1]

        if case == 0: # Case 0: get summary and set summary to textedit_summary
            print("This is case 0")
            self.textEdit_source.setPlainText(input_text)
            self.import_reference.setText(input_text_path)
